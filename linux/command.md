...
این راهنما دستورات اساسی لینوکس را به همراه نحوه استفاده آنها به صورت دسته‌بندی شده برای مرجع سریع ارائه می‌دهد.
...
- **`cat`**: نمایش محتویات فایل.
  ...
  cat file.txt
  ```
- **دستورات مرتبط با `grep`**: جستجو در خروجی دستورات با استفاده از لوله (pipe).
```bash 
ls -l | grep "pattern"  
```
...
پاک شدد همه نیازممندی ها سیستم بلا استفاده
...
این راهنما شامل دستورات پایه تا پیشرفته است و پایه‌ای محکم برای کار با سیستم‌های لینوکس به طور مؤثر فراهم می‌کند.
